...
class CandidateController {
  async dashboard(req, res) {
    try {
      ...
      const candidateDetail = await candidateRepositories.findCandidateById(
        userId
      );
      const {
        applicationsCount,
      ...
      } = await applicationRepositories.getApplicationCounts(userId);
      const { recommendedJobs, totalRecommendedJobs } =
        await jobRepositories.getRecommendedJobs(userId);
  ...
  async getCandidateprofileData(req, res) {
    try {
      ...
      const jobCategoriesToselect =
        await jobCategoryRepositories.getAllJobCategories();

      const candidateBasic =
        await candidateRepositories.getCandidateBasicDetails(id);
      const candidateProfileSummary =
        await candidateRepositories.getCandidateProfileSummary(id);
      const candidateSkills = await candidateRepositories.getCandidateSkills(
        id
      );
      const candidateWorkExperiences =
        await candidateRepositories.getCandidateWorkExperiences(id);
      const candidateEducations =
        await candidateRepositories.getCandidateEducations(id);
      const candidatePreferences =
        await candidateRepositories.getCandidateCareerPreference(id);
      const candidateResume = await candidateRepositories.getCandidateResume(
        id
      );
      const candidateProjects =
        await candidateRepositories.getCandidateProjects(id);
  ...
  async getCandidateBasicDetails(req, res) {
    try {
      const id = req.user._id;
      const candidate = await candidateRepositories.getCandidateBasicDetails(
        id
      );
  ...
  async updateCandidateBsicDetail(req, res) {
    try {
      ...
      const { error, value } = basicDetailsSchema.validate(req.body, {
        abortEarly: false,
      });
      if (error) {
        const errors = {};
      ...
      const updatedCandidate =
        await candidateRepositories.updateCandidateBasicDetails(userId, value);
      if (!updatedCandidate) {
  ...
  async updateProfileImage(req, res) {
    try {
      const candidate = await candidateRepositories.findCandidateById(
        req.user._id
      );
      ...
      const updatedCandidate =
        await candidateRepositories.updateCandidateProfilePicture(
          req.user._id,
          profilePicture
        );
  ...
  async updateCandidateProfileSummary(req, res) {
    try {
      ...
      const userId = req.user._id;
      const updatedCandidate =
        await candidateRepositories.updateCandidateProfileSummary(
          userId,
          profileSummary
        );
      if (!updatedCandidate) {
  ...
  async updateCandidateSkills(req, res) {
    try {
      ...
      const updatedCandidate =
        await candidateRepositories.updateCandidateSkills(userId, skills);
      if (!updatedCandidate) {
        return res.status(404).json({
          status: false,
          message: "Candidate is not found.",
  ...
  async addCandidateEducation(req, res) {
    try {
      ...
      const educationValidationSchema = Joi.object({
        level: Joi.string()
          ...
          .messages({
            ...
            "any.only":
              "Education level must be one of Tenth, Twelfth, Diploma, Graduation, Post Graduation, PhD, or Other.",
            "string.base": "Education level must be a string.",
          }),

        boardOrUniversity: Joi.string().required().messages({
        ...
        marksPercentage: Joi.number().min(0).max(100).messages({
        ...
        durationFrom: Joi.string().allow("", null).messages({
          "string.base": "Duration from must be a string.",
        }),

        durationTo: Joi.string().allow("", null).messages({
          "string.base": "Duration to must be a string.",
        }),
      });

      const { error, value } = educationValidationSchema.validate(req.body, {
        abortEarly: false,
      });
      if (error) {
        console.log(error);
      ...
      const duplicate = existingEducations.some((edu) => edu.level === level);
      ...
      const updatedCandidate =
        await candidateRepositories.addCandidateEducation(userId, newEducation);

      const courseDoc = newEducation.course
        ? await courseRepositories.getCourseById(newEducation.course)
        : null;
      const specializationDoc = newEducation.specialization
        ? await specializationRepositories.getSpecializationById(
  ...
  async deleteCandidateEducation(req, res) {
    try {
      ...
      const updatedCandidate =
        await candidateRepositories.deleteCandidateEducation(userId, eduId);
      if (!updatedCandidate) {
        return res.status(404).json({
          status: false,
          message: "Candidate is not found.",
  ...
  async addCandidateWorkExperience(req, res) {
    try {
      const workExperienceSchema = Joi.object({
        ...
        currentEmployment: Joi.boolean().required().messages({
          "boolean.base":
            "Current employment status must be a boolean (true/false).",
        ...
        workTillDate: Joi.date()
          ...
          .when("currentEmployment", {
            ...
            then: Joi.date().required().messages({
              ...
              "any.required":
                "Work till date is required if you are not currently employed.",
            }),
        ...
        skillsUsed: Joi.array().items(Joi.string()).messages({
          "array.base": "Skills used must be an array.",
          "array.items": "Each skill must be a valid identifier.",
        }),

        jobProfile: Joi.string().trim().max(200).allow("").messages({
          "string.base": "Job profile should be text.",
          "string.max":
            "Job profile description is too long. Please keep it under {#limit} characters.",
        }),
      });
      ...
      const { error, value } = workExperienceSchema.validate(dataToValidate, {
        abortEarly: false,
      });
      if (error) {
        console.log(error);
      ...
      const existingSkills = await skillRepositories.findSkillsByIds(
        dataToValidate.skillsUsed
      );
      if (existingSkills.length !== dataToValidate.skillsUsed.length) {
        return res.status(400).json({
          ...
          errors: {
            skillsUsed:
              "One or more provided skills are invalid or do not exist.",
          },
        });
      }
      ...
      const newWorkExperience = {
        ...
        workTillDate: dataToValidate.workTillDate
          ? new Date(dataToValidate.workTillDate)
          : null,
        skillsUsed: dataToValidate.skillsUsed,
        jobProfile:
          dataToValidate.jobProfile === "" ? null : dataToValidate.jobProfile,
      };

      const userId = req.user._id;

      const updatedCandidate =
        await candidateRepositories.addCandidateWorkExperience(
          userId,
          newWorkExperience
        );

      const addedworkExperience =
        updatedCandidate.profile.workExperience[
          updatedCandidate.profile.workExperience.length - 1
        ];
  ...
  async deleteCandidateWorkExperience(req, res) {
    try {
      ...
      const updatedCandidate =
        await candidateRepositories.deleteCandidateWorkExperience(
          userId,
          workExpId
        );
      if (!updatedCandidate) {
  ...
  async updateCareerPreferences(req, res) {
    try {
      ...
      const { error, value } = careerPreferencesSchema.validate(req.body, {
        abortEarly: false,
      });
      if (error) {
        console.log(error);
      ...
      const existingIndustry = await industryRepositories.getIndustryById(
        preferredIndustry
      );
      if (!existingIndustry) {
        return res.status(400).json({
      ...
      const existingJobcategory =
        await jobCategoryRepositories.getJobCategoryById(preferredJobCategory);
      ...
      const existingLocations = await locationRepositories.findLocationsByIds(
        preferredLocations
      );
      if (existingLocations.length !== preferredLocations.length) {
        return res.status(400).json({
          status: false,
          errors: {
            preferredLocations: "one or more locations do not exists.",
          },
        });
      }
      ...
      const updatedCandidate =
        await candidateRepositories.updateCareerPreferences(userId, value);
      res.status(200).json({
        status: true,
  ...
  async getCandidatecarrerPeference(req, res) {
    try {
      ...
      const preferences =
        await candidateRepositories.getCandidateCareerPreference(userId);
  ...
  async updateCandidateResume(req, res) {
    try {
      ...
      let resumePath = req.file
        ? `uploads/resume/${req.file.filename}`
        : candidate.profile?.resume?.path;
      ...
      const updatedCandidate = await candidateRepositories.updateResume(
        req.user._id,
        resume
      );
  ...
  async addCandidateProject(req, res) {
    try {
      ...
      const { error, value } = pojectSchemaValidation.validate(req.body, {
        abortEarly: false,
      });
      if (error) {
        console.log(error);
      ...
      const updatedCandidate = await candidateRepositories.addCandidateProject(
        userId,
        newProject
      );

      const addedProject =
        updatedCandidate.profile.projects[
          updatedCandidate.profile.projects.length - 1
        ];
  ...
  async deleteCandidateProject(req, res) {
    try {
      ...
      const updatedCandidate =
        await candidateRepositories.deleteCandidateProject(userId, projectId);
      if (!updatedCandidate) {
        return res.status(404).json({
          status: false,
          message: "Candidate is not found.",
  ...
  async getCandidatesPaginated(req, res) {
    try {
      ...
      const { candidates, totalCount, totalPages } =
        await candidateRepositories.getCandidatesPaginated(options, filters);
  ...
  async deactivateCandidate(req, res) {
    try {
      ...
      const updatedcandidate = await candidateRepositories.deactivateCandidate(
        candidateId
      );
  ...
  async activateCandidate(req, res) {
    try {
      ...
      const updatedcandidate = await candidateRepositories.activateCandidate(
        candidateId
      );
  ...
  async removeCandidate(req, res) {
    try {
      ...
      const updatedcandidate = await candidateRepositories.removeCandidate(
        candidateId
      );
